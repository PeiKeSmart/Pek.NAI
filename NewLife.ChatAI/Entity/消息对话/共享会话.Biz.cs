...
public partial class SharedConversation : Entity<SharedConversation>
{
    #region 对象操作
    // 控制最大缓存数量，Find/FindAll查询方法在表行数小于该值时走实体缓存
    private static Int32 MaxCacheCount = 1000;

    static SharedConversation()
    {
    ...
    public override Boolean Valid(DataMethod method)
    {
        ...
        // 检查唯一索引
        // CheckExist(method == DataMethod.Insert, nameof(ShareToken));
    ...
    //    if (XTrace.Debug) XTrace.WriteLine("开始初始化SharedConversation[共享会话]数据……");

    //    var entity = new SharedConversation();
    //    entity.ConversationId = 0;
    //    entity.ShareToken = "abc";
    //    entity.SnapshotMessageId = 0;
    //    entity.CreatorUserId = 0;
    //    entity.ExpireTime = DateTime.Now;
    //    entity.Insert();

    //    if (XTrace.Debug) XTrace.WriteLine("完成初始化SharedConversation[共享会话]数据！");
    //}
    ...
    // Select Count(Id) as Id,Category From SharedConversation Where CreateTime>'2020-01-24 00:00:00' Group By Category Order By Id Desc limit 20
    //static readonly FieldCache<SharedConversation> _CategoryCache = new(nameof(Category))
    //{
    ...
    /// <summary>根据会话编号集合批量查找共享会话</summary>
    /// <param name="convIds">会话编号集合</param>
    /// <returns>共享会话列表</returns>
    public static IList<SharedConversation> FindAllByConversationIds(Int64[] convIds)
    {
