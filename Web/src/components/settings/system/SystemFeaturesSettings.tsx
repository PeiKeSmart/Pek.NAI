...
export function SystemFeaturesSettings({ settings, onChange }: Props) {
  const { t } = useTranslation()

  return (
    <div className="space-y-1">
      <Toggle
        checked={settings.enableUsageStats}
        onChange={(v) => onChange({ enableUsageStats: v })}
        label={t('systemSettings.features.enableUsageStats')}
        description={t('systemSettings.features.enableUsageStatsDesc')}
      />
      <Toggle
        checked={settings.backgroundGeneration}
        onChange={(v) => onChange({ backgroundGeneration: v })}
        label={t('systemSettings.features.backgroundGeneration')}
        description={t('systemSettings.features.backgroundGenerationDesc')}
      />
      <div className="py-3">
        <label className="block text-sm font-medium text-gray-900 dark:text-gray-100 mb-1">
          {t('systemSettings.features.maxMessagesPerMinute')}
        </label>
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">{t('systemSettings.features.maxMessagesPerMinuteDesc')}</p>
        <input
          type="number"
          min={0}
          value={settings.maxMessagesPerMinute}
          onChange={(e) => onChange({ maxMessagesPerMinute: Number(e.target.value) })}
