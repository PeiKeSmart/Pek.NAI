...
export function SystemLearningSettings({ settings, onChange }: Props) {
  ...
  return (
    <div className="space-y-1">
      <Toggle
        ...
        onChange={(v) => onChange({ enableAutoLearning: v })}
        label={t('systemSettings.learning.enableAutoLearning')}
        description={t('systemSettings.learning.enableAutoLearningDesc')}
      />
      <div className="py-3">
      ...
      <div className="py-3">
        ...
        <input
          type="number"
          min={0}
          value={settings.minLearningContentLength}
          onChange={(e) => onChange({ minLearningContentLength: Number(e.target.value) })}
          className="w-full px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50"
        />
      </div>
    </div>
