...
export function UserProfile({ name, avatarUrl, isSystem, onSettingsClick, onSystemSettingsClick, onAdminClick, onLogoutClick, className }: UserProfileProps) {
  ...
  return (
    <div ref={containerRef} className={cn('p-3 border-t border-[var(--color-border-subtle)] relative', className)}>
      ...
      {open && (
        <div className="absolute bottom-full left-3 right-3 mb-1 bg-[var(--color-surface-0)] border border-[var(--color-border-subtle)] rounded-xl shadow-menu py-1 z-50">
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"
          >
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center space-x-3 px-2 py-2 rounded-lg hover:bg-gray-200/50 dark:hover:bg-gray-700/50 text-sm text-gray-700 dark:text-gray-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
      >
